...
{
	ocf_cache_line_t prev_cache_line = ocf_metadata_hash_get_hash(cache, hash);
	...
	/* Setup new node */
	ocf_metadata_hash_set_core_info(cache, cache_line, core_id,
			core_line);
	 ...
	 */
	ocf_metadata_hash_set_collision_info(cache, cache_line, prev_cache_line,
			line_entries);

	/* Update previous head */
	if (prev_cache_line != line_entries) {
		ocf_metadata_hash_set_collision_prev(cache, prev_cache_line,
				cache_line);
	}
	 ...
	 */
	ocf_metadata_hash_set_hash(cache, hash, cache_line);
}
		...
		ocf_cache_line_t line, ocf_part_id_t part_id)
{
	...
	ocf_metadata_hash_get_collision_info(cache, line, &next_line, &prev_line);

	/* Update previous node if any. */
	if (prev_line != line_entries)
		ocf_metadata_hash_set_collision_next(cache, prev_line, next_line);

	/* Update next node if any. */
	if (next_line != line_entries)
		ocf_metadata_hash_set_collision_prev(cache, next_line, prev_line);

	ocf_metadata_hash_get_core_info(cache, line, &core_id, &core_sector);
	...
	if (ocf_metadata_hash_get_hash(cache, hash_father) == line)
		ocf_metadata_hash_set_hash(cache, hash_father, next_line);

	ocf_metadata_hash_set_collision_info(cache, line,
