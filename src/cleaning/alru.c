		...
		unsigned int collision_index)
{
	...
	if (env_atomic_read(&part->runtime->cleaning.policy.alru.size) == 0) {
		...
		alru = (struct alru_cleaning_policy_meta *)ocf_metadata_hash_get_cleaning_policy_meta(cache, collision_index);
		...
		alru = (struct alru_cleaning_policy_meta *)ocf_metadata_hash_get_cleaning_policy_meta(cache, curr_head_index);
		...
		unsigned int collision_index)
{
	 ...
	 */
		...
		next_alru = (struct alru_cleaning_policy_meta *)ocf_metadata_hash_get_cleaning_policy_meta(cache, next_lru_node);
	 ...
	 */
		...
		prev_alru = (struct alru_cleaning_policy_meta *)ocf_metadata_hash_get_cleaning_policy_meta(cache, prev_lru_node);
		...
		uint32_t cache_line)
{
	...
	alru = (struct alru_cleaning_policy_meta *)ocf_metadata_hash_get_cleaning_policy_meta(cache, cache_line);
...
static int get_data_to_flush(struct alru_context *alru)
{
	...
	struct alru_cleaning_policy_config *config;
	struct alru_cleaning_policy_meta *meta;
	ocf_cache_line_t cache_line;
	struct ocf_user_part *part;
	...
	for_each_part(cache, part, part_id) {
		...
		OCF_DEBUG_PARAM(cache, "Last access=%u, timestamp=%u rel=%d",
				last_access, meta->timestamp,
				meta->timestamp < last_access);
		...
		while (more_blocks_to_flush(cache, cache_line, last_access)) {
			...
			meta = (struct alru_cleaning_policy_meta *)ocf_metadata_hash_get_cleaning_policy_meta(cache, cache_line);
			cache_line = meta->lru_prev;
		}
